...
- The branch is not `aether-agent`; the pull failed before the patch was made. This must remain a handoff blocker, not a hidden success condition.
...
- REVISE for the overall LOOP-01 exit criteria: install/use ast-grep or Semgrep, establish a current branch from remote, and address or explicitly defer the non-streaming timeout and vault-size findings.
