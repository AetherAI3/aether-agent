...
import {
  goalsForWorkspace, getGoal, getActiveGoal, newGoal, newPhase, newTask,
...
export async function handleGoal(
  ...
  switch (subcmd) {
    ...
    case "start": {
      const id = rest.trim();
      const goal = id ? getGoal(id) : getActiveGoal(ctx.flags.cwd) ?? goalsForWorkspace(ctx.flags.cwd)[0];
